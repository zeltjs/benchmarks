#!/usr/bin/env node

import { access } from 'node:fs/promises'
import { fork } from 'node:child_process'
import ora from 'ora'
import { join } from 'node:path'
import { fire } from './autocannon.js'
import { fileURLToPath } from 'node:url'
import assert from 'node:assert'

const __dirname = fileURLToPath(new URL('.', import.meta.url))

const doBench = async (opts, handler) => {
  const spinner = ora(`Started ${handler}`).start()
  let forked
  const benchDir = join(__dirname, '..', 'benchmarks')

  // Try .cjs, then .mjs, then subdirectory with dist/main.js (for pre-built TS frameworks)
  try {
    await access(join(benchDir, handler + '.cjs'))
    forked = fork(join(benchDir, handler + '.cjs'))
  } catch {
    try {
      await access(join(benchDir, handler + '.mjs'))
      forked = fork(join(benchDir, handler + '.mjs'))
    } catch {
      // Subdirectory with pre-built dist/main.js
      forked = fork(join(benchDir, handler, 'dist', 'main.js'))
    }
  }

  try {
    spinner.color = 'magenta'
    spinner.text = `Warming ${handler}`
    await fire(opts, handler, false)
  } catch (error) {
    return console.log(error)
  } finally {
    spinner.color = 'yellow'
    spinner.text = `Working ${handler}`
  }

  try {
    await fire(opts, handler, true)
    assert.ok(forked.kill('SIGINT'))
    spinner.text = `Results saved for ${handler}`
    spinner.succeed()
    return true
  } catch (error) {
    return console.log(error)
  }
}

let index = 0
const start = async (opts, list) => {
  if (list.length === index) {
    return true
  }

  try {
    await doBench(opts, list[index])
    index += 1
    return start(opts, list)
  } catch (error) {
    return console.log(error)
  }
}

export default start
