...
const packages = {
  ...
  'nestjs-express': { hasRouter: true, package: '@nestjs/core' },
  'nestjs-fastify': { hasRouter: true, package: '@nestjs/core' },
  ...
  'zeltjs': { hasRouter: true, package: '@zeltjs/core' },
}
...
Object.keys(packages).forEach(pkg => {
  if (!packages[pkg].version) {
    ...
    try {
      ...
      // For subpackages (nestjs-*, zeltjs), try to get version from their own node_modules
      try {
        const version = require(resolve(`benchmarks/${pkg}/node_modules/${module}/package.json`)).version
        packages[pkg].version = version
      } catch {
        packages[pkg].version = 'local'
      }
    }
  }
  _choices.push(pkg)
